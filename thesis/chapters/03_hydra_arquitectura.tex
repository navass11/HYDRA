...
\node[module=blueblock] (rain) at (-5.0,2.35) {\textbf{Meteorología}\\ERA5, GPM, PERSIANN\\AEMET, OGIMET, Meteostat};
...
\coordinate (modelBus) at (0,-0.30);
\draw[softarr] (users.south) -- (0,3.08);
\foreach \n in {rain,river,clim,soil} {
  \draw[softarr] (\n.south) -- ++(0,-0.23);
}
\draw[arr] (core.south) -- (modelBus);
\foreach \n in {hms,swat,ras,sfincs} {
  \draw[arr] (modelBus) -| (\n.north);
  \draw[softarr] (\n.south) -- ++(0,-0.22);
}
\draw[arr] (modelBus) -- (impact.north);
...
El paquete \pyhydra está disponible como repositorio propio en \href{https://github.com/navass11/pyhydra}{GitHub: \texttt{navass11/pyhydra}} e instalable mediante \texttt{pip install -e .} desde su raíz. La versión 0.1.0 se ha archivado como software en Zenodo \parencite{navas2026pyhydra}, que constituye la referencia durable del núcleo Python. Su estructura sigue el patrón estándar de paquetes Python modernos: módulos temáticos con \texttt{\_\_init\_\_.py} que exponen la API pública de cada subpaquete, y un fichero \texttt{utils.py} en la raíz con funciones transversales. El repositorio de integración \hydra, disponible en \href{https://github.com/navass11/HYDRA}{GitHub: \texttt{navass11/HYDRA}} \parencite{navas2026hydrarepo}, consume ese paquete como núcleo y añade las capas de ejecución, web y documentación reproducible. La instancia desplegada durante la tesis en \href{https://hydra-web.yellowwave-5aaa93b0.spaincentral.azurecontainerapps.io/}{Azure Container Apps} se trata únicamente como endpoint efímero de demostración y validación operativa; no se usa como referencia durable.
...
\begin{longtable}{p{0.42\textwidth}p{0.12\textwidth}p{0.32\textwidth}}
  ...
  \texttt{climate.bias\_correction} & Clima & Delta method, quantile mapping (EQM, DQM) \\[2pt]
  ...
  \texttt{modeling.hydraulic.sfincs} & Modelos & Configuración, ejecución y lectura de SFINCS \\[2pt]
...
\node[block=tealblock] (climate) at (0,0)
  {{\centering\textbf{Clima y estadística}\par}
   ...
   Generación: NEOPRENE, CoSMoS\\[3pt]
...
\draw[ret] (p5.north) to[out=105,in=75] node[above, font=\scriptsize, color=tg55] {diagnóstico y recalibración} (p2.north);
\end{tikzpicture}
}
...
\node[box=tealblock] (actions) at (-1.75,2.25) {\textbf{GitHub Actions}\\build Linux/amd64};
\node[box=tealblock] (registry) at (1.75,2.25) {\textbf{Registro}\\hydra-web\\hydra-api\\hydra-jupyter};
...
\node[wide=blueblock] (files) at (0,-1.55) {\textbf{Azure Files / volumen de datos}\\notebooks de sesión, proyectos externos y resultados};
...
\draw[soft] (aca.south) |- (web.north);
\draw[soft] (aca.south) -- (api.north);
\draw[soft] (aca.south) |- (jupyter.north);
\draw[arr] (api) -- (files);
\draw[arr] (jupyter) -- (files);
\draw[arr] (user) -- (copy);
\draw[arr] (copy) -- (run);
\draw[soft] (copy) -- (files);
\draw[soft] (run) -- (jupyter);
\end{tikzpicture}
}
...
El arranque completo del entorno se realiza con un único comando desde el directorio raíz del repositorio:

\begin{codeblock}
docker compose -f docker/docker-compose.yml up -d
\end{codeblock}

Para apuntar el volumen de datos a un directorio distinto del repositorio (por ejemplo, un recurso compartido con proyectos de modelización):
...
\subsection{Instalación directa sin Docker}

Para instalación directa en el entorno Python del usuario, sin los servicios de frontend ni backend:

\begin{codeblock}
pip install -e .
\end{codeblock}

Esta vía es la recomendada para desarrollo de módulos propios e iteración rápida sobre el código fuente. Las dependencias extendidas (PyMC, pyvinecopulib, etc.) pueden instalarse selectivamente según los módulos que se necesiten.

El entorno Docker fija las versiones principales del entorno Python y reduce la variabilidad asociada al sistema operativo o a las dependencias del usuario. Los resultados que dependen de servicios externos, credenciales, datos actualizados o motores hidráulicos instalados en el host siguen requiriendo registrar la versión y la configuración empleadas.
...
\begin{table}[p]
  ...
  \midrule
  T1 & Generación estocástica de precipitaciones y análisis de extremos & \path{climate.time_series}, \path{climate.stochastic_generation} --- Caps.~5 y 8 \\[4pt]
  T2 & Eventos de caudal, downscaling híbrido y reconstrucción de inundación & \path{climate.hybrid_downscaling} --- Caps.~5, 6 y 8 \\[4pt]
  T3 & Descarga y corrección de sesgo de datos climáticos & \path{data_sources}, \path{climate.bias_correction} --- Caps.~4, 5 y 7 \\[4pt]
  T4 & Configuración y ejecución automática de modelos hidrológicos e hidráulicos & \path{modeling.hydrology}, \path{modeling.hydraulic} --- Caps.~6 y 7 \\[4pt]
  T5 & Integración, validación, documentación y transferencia & todos los módulos --- Caps.~3, 7, 8 y 9 \\
  \bottomrule
  \end{tabular}
\end{table}
