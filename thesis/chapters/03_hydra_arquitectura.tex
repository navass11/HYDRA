...
\coordinate (sourceBus) at (0,1.58);
\coordinate (modelBus) at (0,-0.30);
\coordinate (impactBus) at (0,-1.68);
\draw[softarr] (users.south) -- (0,3.08);
\draw[bus] (-5.0,1.58) -- (5.0,1.58);
\foreach \n in {rain,river,clim,soil} {
  \draw[softarr] (\n.south) -- (\n |- sourceBus);
}
\draw[arr] (sourceBus) -- (core.north);
\draw[arr] (core.south) -- (modelBus);
\draw[bus] (-4.20,-0.30) -- (4.20,-0.30);
\foreach \n in {hms,swat,ras,sfincs} {
  \draw[arr] (modelBus) -| (\n.north);
  \draw[softarr] (\n.south) -- (\n |- impactBus);
}
\draw[bus] (-4.20,-1.68) -- (4.20,-1.68);
\draw[arr] (impactBus) -- (impact.north);
