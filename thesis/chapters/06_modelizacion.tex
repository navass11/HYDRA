...
\begin{table}[htbp]
  ...
  \begin{tabular}{p{0.30\textwidth}p{0.60\textwidth}}
    ...
    Modelización hidráulica & SFINCS · HEC-RAS; Iber como motor histórico sin adaptador propio en \pyhydra \\[3pt]
    \bottomrule
  \end{tabular}
\end{table}
...
\paragraph{Requisitos de plataforma y versiones.}
Los adaptadores heredan las restricciones de ejecución de cada motor, que deben declararse como condición de reproducibilidad. HEC-RAS se ejecuta mediante \texttt{rascontrol} sobre una instalación local del programa, lo que restringe esa parte del flujo a entornos Windows; HEC-HMS admite ejecución en Windows (API Jython) y en Linux (\texttt{xvfb-run}); SWAT+ y SFINCS distribuyen ejecutables multiplataforma y son los únicos motores integrables por completo en el entorno Docker de \hydra. Cada adaptador registra en los metadatos de ejecución la versión del motor y la ruta del ejecutable empleado, y la matriz de versiones probadas se mantiene en la documentación del repositorio junto a los tests de integración, de modo que un cambio de versión del motor externo sea detectable y no invalide silenciosamente resultados anteriores.
...
\begin{description}
  \item[Preparación de entradas climáticas.] El módulo cubre tres tareas diferenciadas:
  \begin{itemize}
    \item \texttt{write\_precipitation\_file} y \texttt{write\_temperature\_file} convierten series diarias de precipitación y temperatura al formato clásico \texttt{.pcp}/\texttt{.tmp} de SWAT.
    \item \texttt{\seqsplit{write\_swatplus\_precipitation\_files}} y \texttt{\seqsplit{write\_swatplus\_temperature\_files}} generan el equivalente para SWAT+: ficheros por estación e índices \texttt{pcp.cli}/\texttt{tmp.cli}.
    \item \texttt{edit\_file\_cio} conserva el nombre histórico de la función, pero en proyectos SWAT+ recientes localiza y modifica \texttt{time.sim}, que es donde SWAT+ Editor almacena el período de simulación.
  \end{itemize}
  ...
  \item[Lectura de resultados.] \texttt{read\_channel\_sd} lee los ficheros \texttt{channel\_sd} de SWAT+ para frecuencias diaria, mensual, anual y media anual, y normaliza sus cabeceras repetidas; \texttt{read\_swat\_discharge} extrae la serie \texttt{flo\_out} de una unidad de canal concreta.

  \item[Calibración automática con spotpy.] La clase \texttt{SWATModel} envuelve el ciclo de edición de parámetros y ejecución en un único método \texttt{.run\_swat(*params)}, que devuelve el caudal simulado como \texttt{pd.Series}, y expone sus cotas de calibración mediante la propiedad \texttt{parameter\_bounds}. Sobre esa interfaz, la función \texttt{calibrate\_swat\_sceua} aplica el mismo algoritmo SCE-UA de \texttt{spotpy} usado para HEC-HMS, con función objetivo configurable (NSE, PBIAS o RMSE). A diferencia de \texttt{calibrate\_hms\_sceua}, esta función trata un fallo de simulación (bloqueo de SWAT+, error de \texttt{pySWATPlus}) como un ajuste muy pobre en lugar de interrumpir la búsqueda: la evaluación fallida se sustituye por una serie nula que la función objetivo penaliza con un NSE/RMSE extremo, de modo que SCE-UA continúa explorando el espacio de parámetros en vez de abortar.
\end{description}

La lectura implementada se centra en el formato SWAT+ \texttt{channel\_sd}; no debe interpretarse como un lector universal de todas las salidas históricas de SWAT/SWAT+. Otros ficheros de balance o formatos heredados (\texttt{output.rch}, \texttt{output.sub}) pueden seguir requiriendo postprocesado específico del caso.

El patrón conceptual de este flujo ---proyecciones CMIP corregidas de sesgo transformadas en respuesta hidrológica continua--- es el mismo que aplica SIMPCCe \parencite{navas2025simpce} para el análisis de aportaciones a embalses españoles, si bien esa herramienta emula la transformación hidrológica mediante una red neuronal entrenada sobre las series SIMPA en lugar de ejecutar SWAT. El adaptador de SWAT no cuenta todavía con un caso de estudio publicado en esta línea: su validación es de nivel de módulo, mediante los notebooks de ejemplo sobre proyectos de prueba oficiales del modelo, y su papel en la arquitectura es habilitar ese flujo continuo para los estudios que lo requieran.

\section{Modelos hidráulicos}

\subsection{SFINCS}

SFINCS \parencite{leijnse2021sfincs} (Super-Fast INundation of CoastS) es un modelo hidráulico bidimensional de física reducida desarrollado por Deltares. Su formulación simplificada de las ecuaciones de Saint-Venant reduce drásticamente el tiempo de cómputo manteniendo una precisión aceptable para inundación urbana y costera, lo que lo convierte en el motor preferente para flujos estocásticos con alto número de escenarios.
...
En \hydra, HEC-RAS cubre dos necesidades distintas. En modelos 1D, como Calle 30, permite representar redes de conducción, tramos canalizados o sistemas lineales donde la geometría hidráulica está definida por secciones y conectividad. En modelos 2D, como los análisis posteriores del Besaya, permite obtener campos espaciales de calado y velocidad comparables con SFINCS. El adaptador actual prepara escenarios, modifica ficheros de proyecto, lanza ejecuciones y lee series temporales de salida desde DSS mediante \texttt{hecdss}; la extracción espacial completa de resultados 2D se realiza en los flujos de postproceso de los casos, pero no está expuesta como función pública independiente en \texttt{pyhydra.modeling.hydraulic.hec\_ras}.
...
\begin{description}
  ...
  \item[\texttt{read\_ras\_dss\_timeseries} y \texttt{read\_ras\_max\_wsel}] Leen series temporales de resultados HEC-RAS almacenadas en DSS y extraen, en su caso, el valor máximo de una variable como nivel de agua (\texttt{WSEL}). Estas funciones cubren la lectura tabular de series, no la extracción espacial completa de mallas 2D.
\end{description}
...
Iber \parencite{blade2014iber} es un modelo hidráulico bidimensional de superficie libre desarrollado por el CEDEX y la Universidad de A Coruña. En la versión actual de \pyhydra no existe un adaptador propio para Iber: los trabajos iniciales de la línea prepararon hidrogramas y leyeron resultados exportados desde la interfaz gráfica, pero esas operaciones no están expuestas como funciones públicas del paquete. No se ha implementado un adaptador de automatización completo porque Iber no dispone de una API de línea de comandos documentada que permita la ejecución en modo \textit{batch} sin interacción con la interfaz gráfica, lo que hace inviable su uso en flujos estocásticos con cientos o miles de simulaciones. Esta limitación es la razón técnica por la que HEC-RAS y SFINCS, que sí admiten ejecución automatizada mediante subproceso o controlador externo, sustituyeron progresivamente a Iber en los desarrollos posteriores. Su papel en esta memoria es histórico y metodológico, especialmente en el Besaya \parencite{navas2018besaya} y Mallorca \parencite{navas2019mallorca}, donde se empleó para las simulaciones individuales del subconjunto representativo antes de que los adaptadores de HEC-RAS y SFINCS estuvieran disponibles.
