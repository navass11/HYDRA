...
\begin{table}[htbp]
  ...
  \begin{tabular}{p{0.30\textwidth}p{0.60\textwidth}}
    ...
    Análisis de extremos & GEV/GPD · L-momentos · Inferencia bayesiana (Stan, PyMC) \\[3pt]
...
\begin{description}
  \item[Preparación de entradas.] La función \texttt{create\_flow\_series} convierte una \texttt{pandas.Series} de caudal a la estructura interna de HEC-HMS. La clase \texttt{HMSModel} gestiona la escritura de ficheros DSS de precipitación mediante \texttt{pydsstools} y la actualización del fichero de cuenca \texttt{.basin} con los parámetros de la simulación.

  \item[Ejecución.] El método \texttt{.run()} lanza HEC-HMS en modo batch desde la línea de comandos, redirecciona la salida estándar y espera a la finalización. Este método admite la ejecución de múltiples simulaciones en bucle, lo que permite la simulación sistemática del conjunto de eventos estocásticos.

  \item[Calibración automática con spotpy.] La clase \texttt{HMSModel} incorpora soporte nativo para calibración automática de parámetros del modelo hidrológico mediante la librería \texttt{spotpy} \parencite{houska2015spotpy}. Los atributos \texttt{parameter\_names} y \texttt{observed} configuran los parámetros a calibrar y la serie observada de referencia. El método \texttt{run\_hms(*params)} ejecuta el modelo con un vector de parámetros y calcula la función objetivo. Los algoritmos disponibles son PSO (optimización por enjambre de partículas), DREAM (cadenas de Markov diferencial), SCE-UA (barajado complejo evolutivo) y otros implementados por spotpy.
\end{description}
...
El módulo \texttt{modeling.hydrology.swat} gestiona la integración mediante:

\begin{description}
  \item[Preparación de entradas climáticas.] El método \texttt{.prepare\_climate\_inputs} convierte series de precipitación y temperatura (históricas o proyecciones CMIP6 corregidas) al formato de ficheros de texto esperado por SWAT, creando los ficheros \texttt{.pcp} y \texttt{.tmp} con los encabezados correctos para cada punto de la malla o estación.

  \item[Ejecución por escenarios.] El método \texttt{.run()} lanza el ejecutable de SWAT en el directorio del proyecto, y \texttt{.read\_output} lee los ficheros de salida \texttt{output.rch} (caudal) y \texttt{output.sub} (agua en suelo) y los convierte a \texttt{DataFrames} indexados por fecha y unidad de respuesta hidrológica.
\end{description}
