...
\section{Métodos probabilísticos para el análisis del riesgo de inundación}
\label{sec:metodos_probabilisticos}

Este bloque revisa los fundamentos estadísticos y estocásticos sobre los que se apoya la tesis: el modelado estocástico de inundaciones, el análisis de extremos con incertidumbre, el análisis regional y geoestadístico, el análisis multivariante con cópulas y la generación estocástica de precipitación.

\subsection{Modelado estocástico de inundaciones}
...
\subsection{Análisis estadístico de extremos e inferencia bayesiana}
\label{sec:extremos}
...
\subsection{Análisis de frecuencia regional y geoestadística}
\label{sec:rfa}
...
\subsection{Análisis multivariante y cópulas}
\label{sec:cópulas}
...
\section{Datos y escenarios climáticos}
\label{sec:datos_escenarios}

El segundo bloque de la revisión aborda la materia prima de los estudios: las proyecciones de cambio climático y su corrección de sesgo, y las fuentes globales de datos observacionales y de reanálisis.

\subsection{Cambio climático: datos, proyecciones y corrección de sesgo}
...
\section{Integración y automatización de la cadena de cálculo}
\label{sec:integracion_automatizacion}

El tercer bloque revisa las estrategias para convertir los métodos anteriores en flujos operativos: la automatización de los motores de simulación profesionales y el downscaling híbrido que hace computacionalmente viable el análisis estocástico masivo.

\subsection{Automatización de modelos hidrológicos e hidráulicos}
...
La experiencia con HydroMT \parencite{eilander2023hydromt} ha demostrado el valor de construir modelos reproducibles mediante interfaces programáticas. Inspirado en este precedente, \hydra adopta una arquitectura de adaptadores: cada modelo se encapsula en un módulo que traduce datos internos de la librería a los formatos esperados, lanza la ejecución y recupera y homogeniza las salidas. Esto permite organizar flujos de simulación masiva sin alterar los motores físicos ni su aceptación técnica por parte de administraciones y consultoría.

Los casos de aplicación del proyecto FORESEE \parencite{ihcantabria2020foresee,ihcantabria2021m30} y los estudios realizados para el Besaya, Mallorca y Calle 30 \parencite{navas2018besaya,navas2019mallorca,navas2024calle30} han puesto de manifiesto los puntos exactos de fricción en la automatización de cada modelo y han informado las decisiones de diseño de los adaptadores implementados en \pyhydra.

\subsection{Downscaling híbrido y reconstrucción de manchas de inundación}
...
Esta afirmación debe contrastarse con los marcos integrados existentes más próximos. CLIMADA \parencite{aznar2019climada} es la plataforma de referencia para la evaluación global de riesgo climático, pero opera sobre funciones de impacto agregadas y no incorpora la cadena hidrológico-hidráulica explícita ni la generación estocástica de eventos. RainyDay \parencite{wright2017rainyday} resuelve la generación estocástica de tormentas mediante transposición de campos de teledetección, pero delega la simulación física y el análisis de impactos en herramientas externas. El ecosistema wflow \parencite{vanverseveld2024wflow}, junto con HydroMT \parencite{eilander2023hydromt}, automatiza la construcción y ejecución de modelos hidrológicos distribuidos a partir de datos globales, pero no aborda el análisis multivariante de extremos, la generación estocástica de escenarios ni el período de retorno basado en impacto. Los marcos de simulación continua con cadenas de modelos acopladas \parencite{falter2015continuous} demuestran la viabilidad del análisis de riesgo espacialmente coherente de extremo a extremo, pero son implementaciones de investigación ligadas a un dominio y a un equipo concretos, no herramientas transferibles y documentadas. La \cref{tab:marcos_comparativa} sintetiza esta comparación: ninguno de los marcos revisados cubre simultáneamente la adquisición automática de datos, el análisis probabilístico multivariante, la generación estocástica, la automatización de motores profesionales y la estimación de frecuencia sobre el impacto hidráulico, que es precisamente la combinación que \hydra integra.

\begin{table}[htbp]
  \centering
  \caption{Cobertura funcional de \hydra frente a los marcos integrados más próximos. ``Parcial'' indica que la capacidad existe de forma limitada o requiere herramientas externas.}
  \label{tab:marcos_comparativa}
  \small
  \setlength{\tabcolsep}{3pt}
  \begin{tabular}{p{0.34\textwidth}ccccc}
    \toprule
    Capacidad & \hydra & CLIMADA & RainyDay & wflow & Sim.\ continua \\
    \midrule
    Adquisición automática de datos globales & Sí & Sí & Parcial & Sí & --- \\
    Extremos con incertidumbre (bayesiano, AFR) & Sí & Parcial & --- & --- & Parcial \\
    Análisis multivariante (cópulas) & Sí & --- & --- & --- & Parcial \\
    Generación estocástica de eventos & Sí & --- & Sí & --- & Sí \\
    Corrección de sesgo de proyecciones & Sí & Parcial & --- & Parcial & Parcial \\
    Motores profesionales (HEC, SWAT, SFINCS) & Sí & --- & --- & Parcial & --- \\
    Período de retorno basado en impacto & Sí & Parcial & --- & --- & Sí \\
    Producto transferible (Docker, web, notebooks) & Sí & Sí & Parcial & Sí & --- \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{longtable}{p{0.36\textwidth}p{0.07\textwidth}p{0.21\textwidth}p{0.24\textwidth}}
  ...
  \textit{SIMPCCe: A tool for the analysis of reservoir inflows under climate change scenarios} \parencite{navas2025simpce} & 2025 & \textit{Ingeniería del Agua} (publicado) & Climate Change, Bias Correction, Time Series (emulación ANN) \\[4pt]
  \textit{Comparación de métodos de ajuste para la distribución de precipitaciones extremas: Análisis del evento de octubre 2024 en Valencia} \parencite{navas2025valenciaIA} & 2025 & Manuscrito asociado & Extremos (GEV, MLE, MCMC), RFA, incertidumbre \\[4pt]
  \textit{Metodología innovadora para modelar el impacto del cambio climático en el Lago Tanganica con datos globales y regionales} \parencite{navas2025tanganicaIA} & 2025 & Manuscrito asociado & ERA5, CMIP6, Bias Correction, ML, extremos lacustres \\[4pt]
  \textit{From roughness uncertainty to model-structure effects in 2D flood inundation models: a Monte Carlo comparison of SFINCS and HEC-RAS} \parencite{navas2025roughness} & 2025 & Manuscrito asociado & SFINCS, HEC-RAS, Monte Carlo, incertidumbre hidráulica, Besaya \\[4pt]
  \midrule
...
Por tanto, esta memoria no revisa el estado de la técnica desde una posición externa: organiza, formaliza e integra una trayectoria previa de investigación aplicada. \hydra se plantea como la consolidación industrial de esa experiencia en una herramienta modular para estudios de inundación bajo incertidumbre.

\section{Posicionamiento de la tesis frente al estado de la técnica}
