...
\node[source=blueblock] (meteo) at (0,2.35) {\textbf{Lluvia y estación}\\ERA5, GPM, PERSIANN\\AEMET, \mbox{Meteostat}\\OGIMET};
...
\coordinate (sourceBus) at (2.05,0);
\draw[bus] (2.05,-2.30) -- (2.05,2.35);
\foreach \n in {meteo,flow,cmip,phys} {
  \draw[soft] (\n.east) -- (sourceBus |- \n.east);
}
\draw[arr] (sourceBus) -- (std.west);
\coordinate (stdSplit) at (5.95,0);
\draw[arr] (std.east) -- (stdSplit);
\draw[arr] (stdSplit) |- (analysis.west);
\draw[arr] (stdSplit) |- (models.west);
...
\begin{longtable}{p{0.16\textwidth}p{0.20\textwidth}p{0.18\textwidth}p{0.12\textwidth}p{0.20\textwidth}}
  ...
  \label{tab:fuentes} \\
  \toprule
  Fuente & Cobertura & Variable & Resolución & Formato salida \\
  \midrule
  \endfirsthead
  \toprule
  Fuente & Cobertura & Variable & Resolución & Formato salida \\
  ...
  SoilGrids & Global & Textura (arena, limo, arcilla) & 250 m & GeoTIFF \\[2pt]
  \bottomrule
\end{longtable}
}
...
El módulo \texttt{soils.soilgrids} descarga mapas globales de textura del suelo en formato GeoTIFF a 250~m de resolución desde el archivo estático de la versión 2017 de SoilGrids (\texttt{files.isric.org/soilgrids/former}), en las siete profundidades estándar del producto. Se emplea deliberadamente el archivo estático de 2017 en lugar de la versión actual SoilGrids~2.0 \parencite{poggio2021soilgrids} por dos razones operativas: el archivo estático ofrece URLs de descarga directa estables, más robustas para descargas automatizadas por lotes que los servicios WCS con límites de petición de la versión 2.0; y las fracciones granulométricas de ambas versiones son equivalentes a efectos de la clasificación textural USDA que consume la cadena de modelización. La migración del conector a SoilGrids~2.0, que añade cuantificación de incertidumbre por celda, está identificada como mejora futura del módulo. Las variables descargadas son las tres fracciones granulométricas —arena (\texttt{SNDPPT}), limo (\texttt{SLTPPT}) y arcilla (\texttt{CLYPPT})—, suficientes para la clasificación textural USDA que alimenta los flujos de preparación de modelos hidrológicos de \hydra. El pipeline se completa con funciones de clasificación textural (\texttt{find\_usda\_soilclass}) y de agregación modal entre profundidades (\texttt{extract\_mode\_soilclass}) para obtener una única capa de clase de suelo por celda.
...
\begin{itemize}
  ...
  \item datos multidimensionales devueltos como \texttt{xarray.Dataset} con atributos de fuente, fecha de descarga y parámetros de petición;
...
\paragraph{Condiciones de uso y credenciales.}
La reproducibilidad por terceros de las descargas está condicionada por las licencias y los mecanismos de autenticación de cada proveedor. ERA5 y CMIP6 vía CDS requieren aceptar la licencia de Copernicus y disponer de una clave de API personal; el acceso ESGF exige cuenta en un nodo de la federación; GPM-IMERG requiere credenciales de NASA Earthdata; AEMET OpenData funciona con clave de API gratuita sujeta a límites de tasa; GRDC entrega los datos previa solicitud para uso no comercial; y Meteostat, OGIMET, GloFAS, USGS y SoilGrids son de acceso abierto. \pyhydra no almacena credenciales en el código ni en los notebooks: se leen de variables de entorno o de los ficheros de configuración estándar de cada cliente (\texttt{\textasciitilde/.cdsapirc}, \texttt{\textasciitilde/.netrc}), de modo que los flujos publicados son reproducibles sustituyendo únicamente las credenciales personales. Estas condiciones se documentan junto a cada conector y deben tenerse en cuenta al valorar la reproducibilidad de los casos que dependen de fuentes con registro.
