...
\begin{description}
  ...
  \item[\texttt{extract\_peaks\_over\_threshold}] Extrae picos independientes sobre un umbral con separación mínima entre eventos configurable. La separación mínima es el parámetro crítico para garantizar la independencia de los eventos, y debe ajustarse al tiempo de concentración de la cuenca o al tiempo de decaimiento de la variable analizada.

  \item[\texttt{extract\_flood\_events}] Detecta eventos de crecida completos y extrae, para cada uno, el valor pico, la duración y el volumen. Esta función es el punto de entrada para el análisis multivariante mediante cópulas.
\end{description}
...
\begin{description}
  ...
  \item[L-momentos] Estimación por L-momentos de primer a cuarto orden \parencite{hosking1997rfa}. Los L-momentos son más robustos que los momentos clásicos ante la presencia de valores extremos, lo que los hace recomendables cuando la muestra es inferior a 30 años. Se usa como método por defecto cuando no se específica ningún otro.

  \item[Inferencia bayesiana por MCMC] La función \texttt{fit\_gev\_mcmc} ajusta el modelo GEV con Stan \parencite{carpenter2017stan} o PyMC \parencite{salvatier2016pymc} mediante muestreo MCMC. La configuración por defecto usa 4 cadenas con 1000 muestras tras 500 de calentamiento, lo que produce estimaciones de la distribución posterior de los tres parámetros y, por tanto, intervalos de credibilidad completos para cualquier nivel de retorno. Este método es el más costoso computacionalmente, pero produce las estimaciones más informativas sobre incertidumbre.

  \item[Aproximación de Fisher] La función \texttt{fit\_gev\_fisher} estima la incertidumbre paramétrica mediante la matriz de información de Fisher, produciendo muestras de la distribución asintoticamente gaussiana de los parámetros sin recurrir a MCMC. Es un intermedio entre MLE y la inferencia bayesiana completa.
\end{description}

La \cref{fig:gev_comparacion} ilustra el comportamiento comparativo de los tres estimadores sobre un conjunto de 40 máximos anuales generados con parámetros GEV conocidos ($\mu=90$, $\sigma=30$, $\xi=0{,}20$). Las bandas de incertidumbre se obtienen por bootstrap no paramétrico (MLE y L-momentos) y por bootstrap paramétrico (tercer método). La figura sirve como referencia operativa para identificar visualmente las diferencias entre métodos antes de aplicarlos sobre datos reales.
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.88\linewidth]{figures/fig_gev_comparacion}
  \caption{Comparación ilustrativa de los tres métodos de ajuste GEV sobre 40 máximos anuales sintéticos (parámetros reales: $\mu=90$~mm, $\sigma=30$~mm, $\xi=0{,}20$). MLE y L-momentos usan bootstrap no paramétrico (remuestra de los datos); el tercer método usa bootstrap paramétrico (remuestra de la distribución ajustada por MLE). La curva real punteada sirve de referencia. Los tres métodos convergen en el rango observado y divergen a partir de $T \approx 100$ años. Elaboración propia.}
  \label{fig:gev_comparacion}
\end{figure}

Los resultados muestran que ninguna técnica elimina la sensibilidad producida por una observación situada muy lejos del rango histórico. La ventaja específica de la inferencia bayesiana es que representa explícitamente la incertidumbre paramétrica y permite incorporar información previa justificable; no debe interpretarse como una garantía automática de estabilidad. Esta distinción es relevante para el diseño de \hydra: el módulo ofrece varios estimadores para facilitar el diagnóstico y la comparación, no para seleccionar uno de ellos sin evaluar los datos y los supuestos del caso.
...
El módulo \texttt{spatial\_analysis.rfa} implementa el análisis de frecuencia regional (AFR) por el método de Hosking y Wallis \parencite{hosking1997rfa} basado en L-momentos. El procedimiento sigue las etapas estándar:
\begin{enumerate}
  \item Cálculo de L-momentos en cada estación con \texttt{lmom\_fit}.
  \item Verificación de homogeneidad regional mediante el estadístico $H$ de Hosking.
  \item Ajuste de la distribución regional sobre los L-momentos medios ponderados por longitud de registro.
  \item Estimación de factores de frecuencia regionales y reconversión a cuantiles locales mediante el índice de crecida.
\end{enumerate}

La función principal \texttt{regional\_frequency\_analysis} acepta un diccionario de series de estación y la distribución objetivo (GEV por defecto) y devuelve los parámetros regionales y los factores de frecuencia. Su aplicación al caso Valencia 2024 \parencite{navas2025valenciaIA} con tres escalas espaciales (individual, regional local con 9 estaciones, regional global para toda la Comunidad Valenciana) ilustra cómo la elección de la escala de pooling modifica sustancialmente los períodos de retorno estimados del evento.
...
El módulo de interpolación espacial, \path{spatial_analysis.interpolation}, proporciona la función \path{interpolate_spatial} para interpolar campos de estaciones a malla regular. Soporta dos métodos: inverso de la distancia ponderado (IDW) y kriging ordinario o universal con drift externo (elevación, coordenadas). El kriging universal con elevación como variable auxiliar fue el método utilizado en el caso de Mallorca \parencite{navas2019mallorca} para la asignación de precipitación a subcuencas desde los pluviómetros disponibles.
...
El módulo \path{spatial_analysis.bayes_hierarchical} implementa la clase \path{BayesianSpatialModel} para análisis regional con estructura jerárquica. El modelo asume que los parámetros de la distribución de extremos en cada localización son realizaciones de una distribución común de nivel superior, lo que permite transferir información entre estaciones y estimar cuantiles en puntos sin datos. La implementación utiliza PyMC e internamente puede aprovechar la aproximación INLA \parencite{rue2009inla} cuando el tamaño del grafo espacial lo justifica.

\section{Generación estocástica}

\subsection{Precipitación estocástica: NEOPRENE y CoSMoS}

El submódulo \texttt{stochastic\_generation} integra NEOPRENE como librería backend para generación estocástica de precipitación \parencite{diezsierra2023neoprene}. La clase \texttt{NSRPModel} envuelve la implementación puntual del proceso de Neyman-Scott de pulsos rectangulares \parencite{rodriguezituribe1987,rodriguezituribe1988}. El proceso NSRP es un modelo de eventos clusterizados donde cada tormenta madre genera un número aleatorio de células de lluvia con inicio, intensidad y duración extraidas de distribuciones paramétricas. La calibración ajusta los parámetros del proceso para reproducir estadísticos observados de la serie horaria o diaria: media, varianza, probabilidad de precipitación nula, correlación lag-1 y coeficiente de variación. La clase \texttt{STNSRPModel} envuelve la implementación multisitio de NEOPRENE, que extiende NSRP a estaciones correlacionadas y permite la generación de campos de precipitación espacialmente coherentes.
...
La serie temporal de caudal se puede también generar estocásticamente mediante la clase \texttt{CoSMoS} del submódulo \texttt{stochastic\_generation.point}. CoSMoS (\emph{Complete Stochastic Modelling Solution}) genera series sintéticas preservando simultáneamente la distribución marginal, la dependencia temporal y la estacionalidad de la variable simulada \parencite{papalexiou2018storm}. En \pyhydra se utiliza como generador general de series hidrológicas cuando el objetivo es producir realizaciones temporales largas de caudal o precipitación a partir de una estructura estadística calibrada, en contraste con NEOPRENE, que contiene NSRP y STNSRP y representa explícitamente la estructura física de la precipitación mediante tormentas y células.
...
El submódulo \texttt{climate.bias\_correction} proporciona dos métodos para corregir las diferencias sistemáticas entre modelos climáticos y observaciones de referencia \parencite{teutschbein2012biascorrection}:

\begin{description}
  \item[\texttt{delta\_method}] Corrección aditiva (para temperatura) o multiplicativa (para precipitación) que aplica el cambio relativo del modelo climático entre el período histórico de control y el período futuro, preservando el valor medio observado. Es el método más conservador y el preferido cuando se quiere minimizar la interferencia sobre la señal de cambio climático.

  \item[\texttt{quantile\_mapping}] Mapea la distribución empírica del modelo en el período histórico sobre la distribución observada, y aplica la transformación al período futuro. El método EQM (\emph{empirical quantile mapping}) trabaja sobre cuantiles empíricos; DQM (\emph{detrended quantile mapping}) separa previamente la señal de cambio de la variabilidad interanual para preservarla durante la corrección \parencite{cannon2015sdm}; y SDM (\emph{scaled distribution mapping}) preserva explícitamente los cambios proyectados por el modelo climático en variables como precipitación y temperatura \parencite{switanek2017sdm}. Estas variantes son las recomendadas cuando la magnitud del cambio en extremos es relevante.
\end{description}
...
\begin{enumerate}
  \item \textbf{Selección de eventos representativos (MaxDiss).} La función \path{maxdiss}, situada en \path{hybrid_downscaling.classification}, selecciona un subconjunto de $n$ escenarios representativos del espacio total de eventos mediante el algoritmo de máxima disimilitud \parencite{camus2011maxdiss}. El algoritmo construye iterativamente un conjunto que maximiza la distancia mínima entre cualquier par de escenarios, cubriendo el espacio de variabilidad con el mínimo número de simulaciones.
...
\begin{tikzpicture}[
  ...
  arr/.style={-Stealth, line width=0.8pt, tg55},
...
\draw[arr] (events) -- (audit1);
\draw[arr] (maxdiss) -- (audit2);
\draw[arr] (maps) -- (audit3);
...
Este flujo, denominado \emph{downscaling híbrido} porque combina selección estadística con reconstrucción por aprendizaje automático, reduce el coste computacional de la metodología estocástica en varios órdenes de magnitud respecto a simular el conjunto completo de eventos \parencite{deljesus2024downscaling}. Su validez depende de que los descriptores de evento capturen suficientemente las variables de control del modelo hidráulico, y de que el subconjunto MaxDiss sea representativo del espacio completo de variabilidad.
