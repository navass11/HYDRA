...
# Q: pd.DataFrame con columnas ["Q"] e índice DatetimeIndex en UTC
\end{codeblock}
...
\begin{description}
  ...
  \item[\texttt{bias\_correction}.] Corrección de sesgo de proyecciones CMIP6: método delta y mapeo cuantil (EQM, DQM, SDM) implementados en la clase \texttt{BiasCorrection}.
...
# 4. Intervalo de confianza al 90% por bootstrap paramétrico
lower, upper = return_level_ci(
    ams.values, params, T=100, method="bootstrap",
    n_bootstrap=500, alpha=0.10,
)

# 5. Figura de diagnóstico (QQ, PP y curva de retorno)
plot_return_levels(ams.values, params, T_max=500)
\end{codeblock}
...
bc = BiasCorrection(method="delta")
precip_corrected = bc.fit_transform(
