...
El objetivo de este anexo es proporcionar una ruta práctica para ejecutar los flujos principales con el menor número posible de decisiones ambiguas. El usuario puede trabajar de dos formas complementarias: instalando \pyhydra como librería Python, recomendado para desarrollo y uso desde scripts propios; o mediante la plataforma \hydra con Docker, recomendada para reproducir los notebooks, navegar la web y ejecutar los servicios sin configurar manualmente el entorno. La \cref{tab:manual_modos_uso} resume cuándo conviene cada modo.
...
El núcleo instalado con \texttt{pip install -e .} cubre el stack científico base (\cref{tab:dependencias}). Las dependencias extendidas pueden activarse de forma selectiva mediante \emph{extras} declarados en \texttt{pyproject.toml}, sin necesidad del entorno Docker completo:

\begin{codeblock}
pip install "pyhydra[statistics]"   # PyMC, pyvinecopulib, lmoments3
pip install "pyhydra[geospatial]"   # GeoPandas, rasterio, earthaccess
pip install "pyhydra[models]"       # hydromt_sfincs, hecdss, pySWATPlus, spotpy
pip install "pyhydra[all]"          # todas las anteriores
\end{codeblock}

Para reproducir exactamente los resultados de esta memoria, se recomienda fijar la versión etiquetada del paquete en lugar de instalar la rama principal:
...
El servicio \texttt{web} expone una interfaz de navegación de notebooks en el puerto 80. El acceso a JupyterLab se realiza en \texttt{http://localhost/jupyter/} y el backend de herramientas en \texttt{http://localhost/api/}. La \cref{fig:hydra_web_home} muestra la pantalla principal de esta interfaz.
...
Los 26 notebooks generales de ejemplo catalogados por la web, excluyendo casos piloto y material interno de preparación de artículos, funcionan como guías ejecutables de las funcionalidades del paquete.\footnote{Los contadores mostrados en la portada de la interfaz web corresponden al estado del catálogo en el momento de cada captura y pueden diferir del recuento de cierre de esta memoria; la cifra de referencia es la del catálogo fuente \texttt{notebooks.json}: 26 notebooks generales y 23 de casos piloto.} La \cref{tab:notebooks} lista su correspondencia con los módulos, y la \cref{fig:hydra_web_tutorial} muestra la estructura tutorial de la web que conecta ese catálogo con la referencia de API y los casos piloto. El catálogo fuente de la interfaz web contiene además 23 entradas de casos piloto, usadas para recorrer flujos completos de Besaya, M-30, Manning y Valencia desde el navegador.
...
La \cref{tab:manual_responsabilidades} resume la responsabilidad técnica de cada uno de estos tres bloques.
...
La documentación del producto no se apoya solo en la lectura del código fuente. Cada bloque de la librería se valida mediante notebooks que actúan como ejemplos reproducibles: contienen una descripción metodológica, datos sintéticos o de referencia, llamadas a la API y productos de salida. Esta estructura es especialmente importante en una tesis con mención industrial, porque permite demostrar que \hydra no es un prototipo aislado, sino una herramienta con flujos de uso comprobables.

Además de los notebooks funcionales generales, el directorio del caso piloto de Los Corrales de Buelna, situado en \texttt{notebooks/pilot\_cases/}, documenta el origen de la línea metodológica. Este material complementa el capítulo de casos de estudio porque muestra el encadenamiento completo de la librería desde adquisición de datos hasta mapas de período de retorno, tal como detalla la \cref{tab:manual_corrales_notebooks}.
...
La reproducibilidad de \hydra se entiende en términos concretos: qué puede ejecutar un tercero, qué dependencia externa condiciona la ejecución y qué evidencia queda para auditar el resultado. La \cref{tab:reproducibilidad} no es una promesa de tiempos universales, sino una matriz de madurez reproducible de los flujos documentados.

\begin{table}[htbp]
  \centering
  \footnotesize
  \caption{Matriz de reproducibilidad de los flujos documentados en \hydra.}
  \label{tab:reproducibilidad}
  \begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.24\textwidth}>{\raggedright\arraybackslash}p{0.17\textwidth}>{\raggedright\arraybackslash}p{0.24\textwidth}>{\raggedright\arraybackslash}p{0.23\textwidth}@{}}
    \toprule
    Flujo & Nivel reproducible & Dependencia externa & Evidencia auditable \\
    \midrule
    Ajuste GEV/GPD y comparación de niveles de retorno & Completo & Ninguna; datos sintéticos incluidos & Figura 5.1, parámetros ajustados, intervalos y notebooks de diagnóstico \\
    Cópulas e inundación compuesta & Completo & Ninguna; datos sintéticos incluidos & Isolíneas AND/OR y punto de diseño MPDE \\
    Generación estocástica NEOPRENE/CoSMoS & Completo & Librerías instaladas en el entorno Docker & Series sintéticas, semillas y parámetros \\
    Corrección de sesgo e interpolación espacial & Completo con datos de ejemplo & ERA5/CMIP6 o estaciones si se usa caso real & NetCDF/GeoTIFF con metadatos de fuente \\
    Descarga de datos externos & Reproducible bajo credenciales & CDS, NASA Earthdata, AEMET o ESGF según fuente & Ficheros descargados y parámetros de consulta \\
    HEC-HMS, HEC-RAS, SWAT y SFINCS & Reproducible si existe motor/proyecto & Ejecutable y proyecto calibrado del modelo externo & Hidrogramas, mapas o tablas de salida \\
    Casos piloto completos & Parcial por tamaño/licencias & Datos, modelos y credenciales del caso & Resultados trazables y notebooks de auditoría \\
    \bottomrule
  \end{tabular}
\end{table}

Los flujos marcados como completos pueden ejecutarse con el entorno Docker y los datos incluidos o sintéticos. Los flujos condicionados por credenciales, motores externos o proyectos calibrados siguen siendo reproducibles en el sentido de trazabilidad de código, parámetros y salidas, pero no son autocontenidos; esta distinción es esencial para no confundir reproducibilidad computacional con redistribución completa de datos y modelos de terceros.
...
El bloque de modelización expone interfaces de automatización para cuatro motores de simulación externos organizados en dos familias. Iber se documenta en esta memoria por su uso en los casos históricos del Besaya y Mallorca, pero no dispone de adaptador propio en la versión actual de \pyhydra.

\begin{description}
  \item[Hidrología.] \texttt{modeling.hydrology.hec\_hms} envuelve HEC-HMS con funciones de preparación de entradas meteorológicas, escritura en formato DSS, parametrización automática a partir de datos geoespaciales (curva número, tiempo de concentración, Muskingum) y lectura de resultados. \texttt{modeling.hydrology.swat} implementa funciones para preparar ficheros de forzamiento de SWAT/SWAT+, ajustar el período de simulación en \texttt{time.sim}, lanzar el ejecutable y leer salidas \texttt{channel\_sd} de SWAT+; no es todavía un lector universal de todos los ficheros históricos de SWAT.

  \item[Hidráulica.] \texttt{modeling.hydraulic.sfincs} configura proyectos SFINCS desde geometría, DEM y series de caudal, calcula la condición de contorno aguas abajo con Manning y lanza el ejecutable. La lectura de resultados SFINCS en ensemble se realiza actualmente desde \texttt{modeling.hydraulic.sensitivity} mediante \texttt{load\_sfincs\_ensemble}, que extrae \texttt{hmax} de los ficheros \texttt{sfincs\_map.nc}. \texttt{modeling.hydraulic.hec\_ras} modifica ficheros de proyecto no estacionario, inserta hidrogramas de contorno, ejecuta el plan activo y lee series temporales de salida DSS mediante \texttt{hecdss}; la extracción espacial completa de resultados 2D sigue quedando fuera de esa API pública.
\end{description}

El patrón de uso es común, aunque no idéntico, en los módulos implementados: preparar los datos en formatos internos de \pyhydra, llamar a las funciones de configuración, ejecutar el motor y, cuando existe lector implementado, recuperar resultados mediante funciones de postprocesado. SWAT y SFINCS sí cuentan con funciones específicas en \pyhydra; Iber no. Los adaptadores reducen la exposición directa al formato propietario de cada motor y mantienen contratos de entrada y salida compatibles con el análisis estadístico; no eliminan las diferencias entre motores ni sustituyen la revisión técnica de cada modelo. Los notebooks de ejemplo documentan configuraciones completas para los casos del Besaya (HEC-HMS + SFINCS), Calle 30 (HEC-HMS + HEC-RAS) y SIMPCCe (SWAT); la referencia de parámetros se recoge en el \cref{app:api}.
...
\section{Licencia, gobernanza y credenciales}
\label{sec:gobernanza}

\pyhydra y \hydra se distribuyen bajo licencia MIT, una licencia permisiva que autoriza el uso, la modificación y la redistribución ---también con fines comerciales--- con la única condición de conservar el aviso de copyright. Esta elección es coherente con el objetivo de transferencia de la tesis: no impone obligaciones de copyleft a las administraciones, consultoras o grupos de investigación que integren la librería en sus propios flujos, y es la práctica dominante en el ecosistema científico Python en el que se apoya el paquete. Los motores de simulación externos conservan sus licencias propias (dominio público en el caso de las herramientas HEC; código abierto en SWAT+ y SFINCS), que \hydra no redistribuye.

La gobernanza del software sigue un ciclo de desarrollo en repositorios Git con revisión de cambios, publicación de versiones etiquetadas con changelog, y depósito de cada versión estable en Zenodo mediante un DOI de concepto que resuelve siempre a la última versión archivada (\pyhydra: 10.5281/zenodo.20932554, actualmente v0.1.7; \hydra: 10.5281/zenodo.21138150, actualmente v0.1.2), lo que garantiza la citabilidad y la correspondencia verificable entre los resultados de esta memoria y el código que los produce; la página Zenodo de cada DOI de concepto enlaza además a la versión específica de cada release. IHCantabria asume el mantenimiento del proyecto como parte de su cartera de herramientas de transferencia; la distribución en PyPI y la apertura progresiva a contribuciones externas se recogen en el trabajo futuro (\cref{ch:conclusiones}).

Las credenciales de acceso a fuentes de datos (CDS, NASA Earthdata, AEMET, nodos ESGF) no se incluyen nunca en el código, los notebooks ni las imágenes Docker: cada usuario las aporta mediante variables de entorno o los ficheros de configuración estándar de cada cliente, según se detalla en el \cref{ch:fuentes_datos}. Esta separación permite publicar flujos completos sin comprometer secretos y hace que la reproducción por terceros dependa únicamente de obtener sus propias credenciales gratuitas.
