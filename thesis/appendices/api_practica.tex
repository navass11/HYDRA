...
\begin{description}[style=nextline,font=\ttfamily]
  ...
  \item[\apisig{GPMDownloader(folder\_out)}]
  Clase para descarga y apertura de productos GPM-IMERG vía \texttt{earthaccess}. Incluye \path{.set_region(lat_min,lat_max,lon_min,lon_max)}, \path{.search(start_date,end_date)} y \path{.open_dataset()}. Devuelve \texttt{xarray.Dataset}.

  \item[\apisig{PERSSIANDownloader(folder\_out)}]
  Clase para descarga de PERSIANN-CCS diario desde el servidor FTP de UCI. Método principal: \texttt{.download\_daily(start, end)}. Sin valor de retorno.
  ...
  \item[\apisig{AEMETDownloader(netcdf\_folder, stations\_shapefile=None)}]
  Widget Jupyter interactivo para selección y descarga de estaciones AEMET. Devuelve el widget ipyvuetify.

  \item[\apisig{AemetCSVLoader(csv\_path)}]
  Carga y filtra de calidad series descargadas por \texttt{AEMETDownloader}. Método \texttt{.load()} devuelve \texttt{pd.DataFrame}.
  ...
  \item[\apisig{OGIMETDownloader(folder\_out)}]
  Widget interactivo para descarga por lotes desde OGIMET.

  \item[\apisig{OgimetCSVLoader(csv\_path)}]
  Carga y consolida series descargadas por \texttt{OGIMETDownloader}. Método \texttt{.load()} devuelve \texttt{pd.DataFrame}.

  \item[\apisig{download\_meteostat\_series(station\_id, start\_date, end\_date,\\ frequency=\textquotesingle{}daily\textquotesingle{}, variables=None)}]
  ...
  \item[\apisig{download\_best\_meteostat\_series(lat, lon, start\_date, end\_date,\\ radius=50, frequency=\textquotesingle{}daily\textquotesingle{}, variables=None)}]
  Combina búsqueda, puntuación y descarga en una sola llamada. Seleccióna la estación con mejor cobertura de datos dentro del radio. Devuelve tupla \texttt{(series, station\_id, ranking\_df)}.

  \item[\apisig{get\_meteostat\_stations\_nearby(lat, lon, radius=50, limit=100)}]
  Lista estaciones Meteostat próximas a un punto. Devuelve \texttt{pd.DataFrame} con metadatos de estaciones.

  \item[\apisig{score\_meteostat\_stations(stations\_df, start\_date, end\_date,\\ frequency=\textquotesingle{}daily\textquotesingle{}, variables=None)}]
  Puntúa estaciones según fracción de datos disponibles en el período. Devuelve \texttt{pd.DataFrame} con columna \texttt{score}.

  \item[\apisig{find\_station\_by\_wmo(wmo\_code)}]
  Busca una estación Meteostat por su código WMO. Devuelve \texttt{pd.DataFrame}.

  \item[\apisig{MeteostatDownloader(lat, lon, radius=50)}]
  Widget JupyterLab con mapa Leaflet para exploración y descarga interactiva de series Meteostat.
\end{description}
...
\begin{description}[style=nextline,font=\ttfamily]
  ...
  \item[\apisig{download\_usgs(site\_no, start\_date, end\_date,\\ parameter\_code=\textquotesingle{}00060\textquotesingle{}, service=\textquotesingle{}iv\textquotesingle{})}]
  Descarga datos de aforos USGS mediante la API NWIS. \texttt{site\_no} puede ser cadena o lista. \texttt{parameter\_code=\textquotesingle{}00060\textquotesingle{}} corresponde a caudal en pies cúbicos por segundo. Devuelve \texttt{pd.DataFrame}.

  \item[\apisig{get\_usgs\_site\_info(site\_no)}]
  Obtiene metadatos de una o varias estaciones USGS. Devuelve \texttt{pd.DataFrame}.

  \item[\apisig{search\_usgs\_sites(bbox, parameter\_code=\textquotesingle{}00060\textquotesingle{})}]
  Busca estaciones USGS dentro de un bounding box \texttt{(lat\_min, lon\_min, lat\_max, lon\_max)}. Devuelve \texttt{pd.DataFrame}.
\end{description}
...
\begin{description}[style=nextline,font=\ttfamily]
  ...
  \item[\apisig{get\_combination\_if\_complete(model, experiment, varíant, variables)}]
...
\begin{description}[style=nextline,font=\ttfamily]
  ...
  \item[\apisig{BivariateCopula(x, y, family=\textquotesingle{}auto\textquotesingle{})}]
  Análisis de eventos compuestos bivariantes. \texttt{family} acepta \texttt{\textquotesingle{}gumbel\textquotesingle{}}, \texttt{\textquotesingle{}clayton\textquotesingle{}}, \texttt{\textquotesingle{}frank\textquotesingle{}} o \texttt{\textquotesingle{}auto\textquotesingle{}} (selección por AIC). Métodos principales:
  \begin{itemize}
    \item \texttt{.fit()} — Ajusta marginales paramétricas y cópula. Devuelve \texttt{self}.
    \item \texttt{.joint\_return\_period(t, kind=\textquotesingle{}or\textquotesingle{})} — Isolínea AND u OR para período $t$. Devuelve \texttt{(x\_vals, y\_vals)}.
    \item \texttt{.mpde(t)} — Evento de Diseño de Máxima Probabilidad sobre la isolínea de período $t$. Devuelve \texttt{(x\_mpde, y\_mpde)}.
  \end{itemize}

  \item[\apisig{TrivariateCopula(x, y, z, family=\textquotesingle{}auto\textquotesingle{})}]
  Extensión a tres variables del análisis de cópulas Archimedanas. Misma interfaz que \texttt{BivariateCopula}.

  \item[\apisig{VineCopula(data, vine\_type=\textquotesingle{}cvine\textquotesingle{})}]
  Vine cópulas para dependencia multivariante flexible (requiere \texttt{pyvinecopulib}). Permite modelizar dependencia de cola asimétrica entre variables.
...
\begin{description}[style=nextline,font=\ttfamily]
  ...
  \item[\apisig{fit\_distribution(values, dist=\textquotesingle{}gengamma\textquotesingle{}, norm\_type=\textquotesingle{}N1\textquotesingle{},\\ n\_points=30)}]
...
\begin{description}[style=nextline,font=\ttfamily]
  \item[\apisig{HydrographClassifier(n\_types, n\_components=None)}]
  Clasifica formas de hidrograma mediante remuestreo a longitud fija, PCA y \textit{k}-means. \texttt{n\_types} debe ser un cuadrado perfecto (e.g.\ 9, 16, 25) para disposición en rejilla. Método \texttt{.fit\_predict(events\_df, discharge\_series)} devuelve \texttt{pd.DataFrame} con columna de tipo de hidrograma.
...
\begin{description}[style=nextline,font=\ttfamily]
  ...
  \item[\apisig{generate\_manning\_combinations(distributions, n\_samples, ...)}]
  Genera combinaciones Monte Carlo de coeficientes de Manning para análisis de sensibilidad hidráulica.

  \item[\apisig{load\_flood\_ensemble(paths, variable=\textquotesingle{}water\_depth\textquotesingle{}, ...)}]
  Carga un ensemble de mapas de inundación como \texttt{xarray.DataArray} para análisis estadístico espacial.
  ...
  \item[\apisig{manning\_flood\_regression(df, target, predictors)}]
  Ajusta modelos de regresión para relacionar parámetros de Manning y métricas de inundación.

  \item[\apisig{filter\_anomalous\_simulations(ensemble, ...)}]
  Identifica y filtra simulaciones anómalas dentro de un ensemble hidráulico.
\end{description}
