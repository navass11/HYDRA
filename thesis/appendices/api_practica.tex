...
\begin{description}[style=nextline,font=\ttfamily]
  ...
  Carga y consolida series descargadas por \texttt{OGIMETDownloader}. Métodos: \texttt{.load\_station\_data()}, \texttt{.load\_series\_data(variable)} y \texttt{.analyze\_series\_quality()} (mismo patrón que \texttt{AemetCSVLoader}).
...
\begin{description}[style=nextline,font=\ttfamily]
  ...
  Ajusta modelos de regresión para relacionar parámetros de Manning (ensemble) y métricas de inundación (ensemble) derivadas de \texttt{load\_flood\_ensemble}/\texttt{load\_sfincs\_ensemble}. Devuelve \texttt{pd.DataFrame}.
