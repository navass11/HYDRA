...
\begin{description}[style=nextline,font=\ttfamily]
  ...
  Carga las series descargadas por \texttt{AEMETDownloader} y las filtra según criterios de calidad. Métodos: \texttt{.load\_station\_data()} (lee \texttt{selected\_stations.csv}, devuelve \texttt{pd.DataFrame}), \texttt{.load\_series\_data(variable)} (concatena las series de la variable indicada) y \texttt{.analyze\_series\_quality()}.
...
\begin{description}[style=nextline,font=\ttfamily]
  ...
  \item[\apisig{fit\_regional\_gev(data\_dict, method=\textquotesingle{}lmom\textquotesingle{},\\ n\_chains=4, n\_samples=1000)}]
  Ajusta una GEV regional sobre datos agrupados normalizados (formulación index-flood). \texttt{method} acepta \texttt{\textquotesingle{}mle\textquotesingle{}}, \texttt{\textquotesingle{}lmom\textquotesingle{}} o \texttt{\textquotesingle{}bayes\textquotesingle{}}. Devuelve tupla \texttt{(regional\_params, index\_floods\_series)}; para \texttt{method=\textquotesingle{}bayes\textquotesingle{}}, \texttt{regional\_params} es un \texttt{pd.DataFrame} de muestras posteriores.

  \item[\apisig{regional\_return\_levels(data\_dict, T\_values=(2,5,10,20,50,100),\\ method=\textquotesingle{}lmom\textquotesingle{}, credible=0.90, n\_chains=4, n\_samples=1000)}]
  Calcula cuantiles de período de retorno por estación aplicando escalado por índice de avenida. Con \texttt{method=\textquotesingle{}mle\textquotesingle{}} o \texttt{\textquotesingle{}lmom\textquotesingle{}} devuelve un \texttt{pd.DataFrame}; con \texttt{method=\textquotesingle{}bayes\textquotesingle{}} devuelve la tupla \texttt{(median, lower, upper)} de \texttt{DataFrame}.

  \item[\apisig{discordancy\_critical\_value(n\_sites)}]
  Valor crítico tabulado del estadístico de discordancia de Hosking--Wallis para una región con \texttt{n\_sites} estaciones.

  \item[\apisig{regional\_discordancy(data\_dict)}]
  Calcula el estadístico de discordancia $D_i$ por estación a partir de ratios de L-momentos. Devuelve \texttt{pd.DataFrame} con \texttt{Di}, \texttt{critical} y \texttt{discordant}.

  \item[\apisig{regional\_heterogeneity(data\_dict, n\_sim=1000, seed=None)}]
  Calcula el estadístico de heterogeneidad regional $H_1$ mediante simulación Monte Carlo de regiones homogéneas. Devuelve \texttt{dict} con \texttt{H}, \texttt{V\_obs}, \texttt{V\_sim\_mean} y \texttt{V\_sim\_std}. Requiere \texttt{lmoments3}.
...
\begin{description}[style=nextline,font=\ttfamily]
  ...
  Selecciona la mejor distribución univariante por BIC (requiere \texttt{openturns}). Devuelve tupla \texttt{(distribución\_ot, muestra\_ot)}.
...
\begin{description}[style=nextline,font=\ttfamily]
  ...
  \item[\apisig{BiasCorrection.fit\_transform(obs, hist, future, var, method=\textquotesingle{}delta\textquotesingle{}, **kwargs)}]
  Atajo de alto nivel para construir la corrección y aplicarla en una sola llamada. \texttt{method} acepta \texttt{\textquotesingle{}delta\textquotesingle{}}, \texttt{\textquotesingle{}eqm\textquotesingle{}}, \texttt{\textquotesingle{}qdm\textquotesingle{}} o \texttt{\textquotesingle{}sdm\textquotesingle{}}.
\end{description}
...
\begin{description}[style=nextline,font=\ttfamily]
  ...
  Modifica el período de simulación de un proyecto SWAT+ localizando \texttt{time.sim} junto a \texttt{file.cio}. El nombre de la función se conserva por compatibilidad con llamadas anteriores. Sin valor de retorno.
  ...
  \item[\apisig{read\_channel\_sd(output\_dir, freq=\textquotesingle{}day\textquotesingle{}, unit=None)}]
  Lee ficheros SWAT+ \texttt{channel\_sd\_\{day,mon,yr,aa\}.txt}, normaliza cabeceras repetidas y, opcionalmente, filtra una unidad de canal. Devuelve \texttt{pd.DataFrame}; para frecuencias diaria, mensual y anual el índice es temporal.

  \item[\apisig{read\_swat\_discharge(output\_dir, unit, freq=\textquotesingle{}day\textquotesingle{})}]
  Extrae la serie \texttt{flo\_out} de una unidad de canal SWAT+ a partir de \texttt{read\_channel\_sd}. Devuelve \texttt{pd.Series}.

  \item[\apisig{SWATModel(...)}]
  Envuelve la edición de parámetros y la ejecución de un proyecto SWAT+ en un único método \texttt{.run\_swat(*params)}, que devuelve el caudal simulado como \texttt{pd.Series}. Expone sus cotas de calibración mediante la propiedad \texttt{parameter\_bounds}.

  \item[\apisig{calibrate\_swat\_sceua(model, observed, dbname, n\_evals=100,\\ objective=\textquotesingle{}nse\textquotesingle{}, ngs=None)}]
  Calibra un \texttt{SWATModel} mediante SCE-UA de \texttt{spotpy}, análogamente a \texttt{calibrate\_hms\_sceua}. Un fallo de simulación se penaliza como un ajuste extremo en lugar de interrumpir la búsqueda. Devuelve el sampler tras la ejecución.
\end{description}
...
\begin{description}[style=nextline,font=\ttfamily]
  ...
  \item[\apisig{read\_ras\_dss\_timeseries(dss\_path, pathname\_prefix, plan\_name=None)}]
  Lee una serie temporal de salida de HEC-RAS desde DSS mediante \texttt{hecdss}, filtrando por prefijo de pathname y, opcionalmente, por plan. Devuelve \texttt{pd.DataFrame} con columnas \texttt{datetime} y \texttt{value}.

  \item[\apisig{read\_ras\_max\_wsel(dss\_path, pathname\_prefix, plan\_name=None)}]
  Atajo para obtener el máximo de una serie DSS de HEC-RAS, por ejemplo el nivel máximo de agua \texttt{WSEL}. Devuelve \texttt{float}.
