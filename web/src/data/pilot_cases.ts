...
export const pilotCases: PilotCase[] = [
  {
    ...
    approach: {
      es: 'La metodología de los notebooks reproduce el pipeline de Navas et al. (2024): (1) los 1.761 eventos históricos de 17 pluviómetros se caracterizan con cuatro parámetros por estación (Pmax, Pmed, Duración, Tipo de hietograma); (2) PCA + K-Means sobre la forma temporal normalizada del hietograma clasifica los eventos (≥100 tipos); (3) una cópula gaussiana OpenTURNS sobre las 68 variables genera ~1M de eventos sintéticos; (4) el algoritmo MaxDiss selecciona 1.000 eventos representativos; (5) los hietogramas se simulan en HEC-HMS (caudales) y HEC-RAS 1D estacionario (calados en 303 secciones); (6) un regresor k-NN (k=3, espacio log-Q) reconstruye calados en los ~1M eventos; (7) el modelo de Poisson compuesto (λ=5.17 ev/año) transforma frecuencias anuales en calados de diseño para T=2…500 años.',
      en: 'The notebooks reproduce the pipeline of Navas et al. (2024): (1) 1,761 historical events from 17 rain gauges are characterised with four parameters per station (Pmax, Pmed, Duration, hyetograph Type); (2) PCA + K-Means on the normalised temporal shape of the hyetograph classifies events (≥100 types); (3) an OpenTURNS Gaussian copula over 68 variables generates ~1M synthetic events; (4) the MaxDiss algorithm selects 1,000 representative events; (5) hyetographs are simulated in HEC-HMS (peak discharges) and steady-state HEC-RAS 1D (water depths at 303 cross-sections); (6) a k-NN regressor (k=3, log-Q space) reconstructs depths for all ~1M events; (7) the compound Poisson model (λ=5.17 ev/yr) yields design depths at T=2…500 years.',
    },
    steps: [
      {
        number: 1,
      ...
      },
      {
        ...
        description: {
          es: 'Reproducción de la Figura 7 de Navas et al. (2024): scatters para P_27 con los 1.000 eventos seleccionados por MaxDiss (rojo) sobre los ~1M sintéticos (azul). Demostración en vivo del algoritmo MaxDiss: selección de 50 eventos representativos de una muestra de 10.000 en el espacio 4D de P_27, mostrando la cobertura cuasi-uniforme obtenida.',
          en: 'Reproduction of Figure 7 from Navas et al. (2024): scatter plots for P_27 with 1,000 MaxDiss-selected events (red) overlaid on ~1M synthetic (blue). Live demonstration of the MaxDiss algorithm: selection of 50 representative events from 10,000 in the 4D space of P_27, showing the quasi-uniform coverage achieved.',
        },
    ...
    stats: [
      { value: '17', label: { es: 'pluviómetros cuenca M30', en: 'rain gauges M30 catchment' } },
    ...
    references: [
      {
        ...
        description: {
          es: 'Análisis de frecuencia de inundación multivariado mediante cópulas gaussianas sobre series de precipitación de 17 estaciones en el entorno de la M30 (Madrid). Metodología completa: PCA + K-Means + cópula + MaxDiss + HEC-HMS + HEC-RAS + kNN + Poisson compuesto. IHCantabria — Universidad de Cantabria.',
          en: 'Multivariate flood frequency analysis using Gaussian copulas over precipitation series from 17 stations around the M30 (Madrid). Complete methodology: PCA + K-Means + copula + MaxDiss + HEC-HMS + HEC-RAS + kNN + compound Poisson. IHCantabria — University of Cantabria.',
        },
      },
    ],
    figures: [
