...
def ejecution_SFINCS(basins, points, caudal, dem, manning, path_output, bas):
    try:
        # Asegúrate de que sel_basin es un GeoDataFrame
        sel_basin = gpd.GeoDataFrame(basins.iloc[[bas]], geometry='geometry')

        # Crear el modelo
        mod = SfincsModel(root=path_output + T + '/Basin_' + str(basins.loc[bas].Cod_Cuen_H) + "/", mode='w+')

        # Establecer geometría activa
        basins.set_geometry('geometry', inplace=True)
        points.set_geometry('geometry', inplace=True) 

        # Guardar el shapefile de cuencas
        basins[basins.index == bas].to_file(mod.root + "/gis/" + 'file.shp', driver='ESRI Shapefile')

        # Verificar CRS y reproyectar si es necesario
        if points.crs != basins.crs:
            points = points.to_crs(basins.crs)

        # Unión espacial
        points_within = gpd.sjoin(points, sel_basin, how='inner', predicate='within')
        points.loc[points_within.index].to_file(mod.root + '/gis/src.shp', driver='ESRI Shapefile')

        src = points_within

        # Verificar el archivo DEM
        dem_ds = gdal.Open(dem)
        if dem_ds is None:
            raise FileNotFoundError(f"Error al abrir el archivo DEM: {dem}")

        # Crear el directorio de salida si no existe
        output_path = mod.root + "/gis/cut.tif"
        output_dir = os.path.dirname(output_path)

        if not os.path.exists(output_dir):
            os.makedirs(output_dir)

        # Crear el corte con GDAL
        OutTile = gdal.Warp(output_path, 
                            dem_ds,  # Asegúrate de que 'dem_ds' es el objeto GDAL Dataset
        ...
        mod.setup_grid_from_region(region={'geom': mod.root + '/gis/file.shp'}, res=30.83207822, crs=32617)
        ...
        # Configuración de series de tiempo de caudal
        mod.set_config("tref", "20231003 000000")
        mod.set_config("tstart", "20231003 000000")
        mod.set_config("tstop", "20231015 000000")
        mod.set_config('dtmax', 1)
        mod.set_config('storevel', 1)

        time = pd.date_range(start=utils.parse_datetime(mod.config["tstart"]), periods=len(flow), freq='1800s')
        ts = pd.DataFrame(index=time, columns=src.index, data=flow.values)

        # Actualizar forzado en el modelo
        ...
        # # Extraer resultados
        # h_max = extract_result_hmax(f'{mod.root}/gis/dep.tif', f'{mod.root}/sfincs_map.nc', f'{mod.root}/gis/hmax.tif')
        # speed_max = extract_result_speed(f'{mod.root}/gis/dep.tif', f'{mod.root}/sfincs_map.nc', f'{mod.root}/gis/speed.tif')
...
if __name__ == "__main__":
    ...
    path_output  = 'H:/N4C_24062024/Resultados/'
    ...
    for p, per in enumerate(path_caudal):
        ...
        for T in ['Q10','Q50','Q100']: # 'Q10','Q50','Q100'
            ...
            dem = 'H:/N4C_24062024/dem_30_fill_v5_N4C.tif'
            #dem = 'P:/99_BID_ATLAS_PANAMA/INUNDACION/dem_30_fill_v5.tif'
            if os.path.exists(path_output_p+T+'/'+'Basin_'+str(basins.loc[bas].Cod_Cuen_H)+'/gis/hmax.tif'):
                continue
            else:
                try:
                    ejecution_SFINCS(basins,points,caudal,dem,manning,path_output_p,bas)
                except:
