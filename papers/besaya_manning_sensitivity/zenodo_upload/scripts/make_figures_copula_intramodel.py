...
ZENODO_DIR = HERE.parent
DIST_CSV = ZENODO_DIR / "data" / "manning_roughness_coefficients.csv"
REF_CSV  = ZENODO_DIR / "data" / "monte_carlo_combinations.csv"
RES_CSV  = ZENODO_DIR / "data" / "comparison_clean_995.csv"
FIG_DIR  = ZENODO_DIR / "figures"
...
for _, row in df_dist.iterrows():
    ...
    dn,params=best_distribution(vals); d=getattr(stats,dn)
...
gs  = gridspec.GridSpec(1,3,figure=fig,wspace=0.42,
                        left=0.07,right=0.97,top=0.86,bottom=0.39)
...
legend_handles = [
    Line2D([], [], marker="o", ls="none", ms=5.5, color="#e07070",
           alpha=0.6, label=f"High regime (N={hm.sum()})"),
    Line2D([], [], marker="o", ls="none", ms=5.5, color="#2060b8",
           label=f"Low regime (N={lm.sum()})"),
...
for arr,lbl,col,ls in zip(ARRS,RHO_LABELS,COLORS,LS_STYLES):
    ...
    legend_handles.append(Line2D([0],[0],color=col,ls=ls,lw=1.4,
                                 label=f"{lbl} range"))
ax_c.legend(handles=legend_handles, fontsize=7, loc="upper center",
            bbox_to_anchor=(0.5, -0.30), ncol=3,
...
fig.suptitle("Effect of inter-class Manning correlation on spatial-mean roughness "
             "and flood outputs", fontsize=9.5, fontweight="bold")
...
for col, ylabel, ax in [
    ...
    ax.set_title(f"SFINCS — CV={cv(y):.1f}%  (unimodal)", fontweight="bold", fontsize=10)
